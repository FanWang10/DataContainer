...
double Field::get_value(double x, double y, double z)
{
    ...
    return solution.read_at(pt.at(0), pt.at(1), pt.at(2), cell_index[0], cell_index[1], cell_index[2]);
}
...
std::vector<double> Field::get_gradient(double x, double y, double z)
{
    ...
    std::vector<int> cell_index = grid.get_cell(pt);
    return solution.read_gradient_at(x, y, z, cell_index[0], cell_index[1], cell_index[2]);
}
